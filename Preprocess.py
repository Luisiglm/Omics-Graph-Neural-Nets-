...
def mut_prep(folder_i, genes):
    ...
    vt_i = np.zeros((len(mut_files),len(genes),4))
...
def depmap_read_exp_mut(depmap_path, genes_list):
    ...
    vtp = np.zeros((exp_np.shape[0],exp_np.shape[1],4)) # variant type.
    ...
    Obs = exp_np.shape[0]*No_crispr
    # pre-allocate memory for 'simulated' expression.
    pos_norm, min_max = pos_normal(pos_muts)
    encode = mut_encoding(pos_norm, 10)
    enc_crispr = np.zeros((Obs, len(genes_list),encode.shape[2]))
    exp_crispr = np.zeros((Obs, len(genes_list)))
    ns_crispr = np.zeros((Obs, len(genes_list)))
    subs_crispr = np.zeros((Obs, len(genes_list),8))
    vtp_crispr = np.zeros((Obs, len(genes_list),4))
    # scores
    scores_crispr = np.zeros((Obs,))
    ii = 0
    for i in range(1,len(lineList)):
        ...
        if depmap_id.count(depmap_i)>0:
            ...
            for j in range(len(genes_list)):
                if cols_crispr_index[j]!= None:
                    exp_crispr[ii,:] = exp_np[J,:]+0.
                    exp_crispr[ii,j] = 0.
                    enc_crispr[ii,:,:] = encode[J,:,:]+0.
                    ns_crispr[ii,:] = ns[J,:] + 0.
                    subs_crispr[ii,:] = subs[J,:] + 0.
                    vtp_crispr[ii,:] = vtp[J,:] + 0.
                    if div[cols_crispr_index[j]] != '':
                        exp_crispr[ii,:] = exp_np[J,:]+0.
                        exp_crispr[ii,j] = 0.
                        scores_crispr[ii] = div[cols_crispr_index[j]]                 
    ...
    exp_crispr = exp_crispr[0:ii,:]
    scores_crispr = scores_crispr[0:ii]
    return(exp_np, exp_crispr, scores_crispr, depmap_id, enc_crispr, vtp_crispr, subs_crispr, ns_crispr)
            ...
            if (np.var(num_array[:,i])>0):
                if (len(num_array.shape) == 2):
                    ...
                    for j in range(num_array.shape[2]):
                        num_array[:,i,j] = (num_array[:,i,j] - np.mean(num_array[:,i,j]))/np.var(num_array[:,i,j])**.5
    elif axis == 2:
